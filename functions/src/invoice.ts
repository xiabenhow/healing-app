...
interface InvoiceCreateRequest {
  ...
  amount: number;
  description?: string;
  carrierType: "cloud" | "phone" | "citizen"; // 雲端、手機號碼、自然人憑證
  carrierNum?: string;
  invoiceType: "personal" | "donate" | "company"; // 個人、捐贈、公司
  companyTaxId?: string; // 統一編號（公司發票時必需）
  items?: Array<{
    name: string;
    quantity: number;
    unitPrice: number;
...
router.post("/create", async (req, res) => {
  try {
    const {
      ...
      companyTaxId,
      items,
    } = req.body as InvoiceCreateRequest;
    ...
    if (items && items.length > 0) {
      const itemsString = items
        .map(
          (item) =>
            `${item.name}*${item.quantity}*${item.unitPrice}*0|`
        )
        .join("");
